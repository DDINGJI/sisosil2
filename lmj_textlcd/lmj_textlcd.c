...
#define DEVICE_NAME			"lmj_textlcd"
...
// ------------------------------------
static int lmj_textlcd_open(struct inode * inode, struct file * file){
	if (textlcd_usage == -1)
			return -EBUSY;
...
static int lmj_textlcd_release(struct inode * inode, struct file * file){
...
/* not use
static ssize_t lmj_textlcd_read(struct file * file, char * buf, size_t length, loff_t * ofs){
	printk("lmj_textlcd_read, \n");
	
	return 0;
}
*/

static ssize_t lmj_textlcd_write(struct file * file, const char * buf, size_t length, loff_t * ofs){
	int i;
		int size;
...
static DEFINE_MUTEX(lmj_textlcd_mutex);
static long lmj_textlcd_ioctl(struct file * file, unsigned int cmd, unsigned long arg){
	switch(cmd){
		case TEXTLCD_ON:
		...
		default:
			mutex_unlock(&lmj_textlcd_mutex);
			return ENOTTY;
		}
...
static struct file_operations lmj_textlcd_fops = {
	.owner = THIS_MODULE,
	.open = lmj_textlcd_open,
	.release = lmj_textlcd_release,
//	.read = lmj_textlcd_read,
	.write = lmj_textlcd_write,
	.unlocked_ioctl = lmj_textlcd_ioctl,
};

static struct miscdevice lmj_textlcd_driver = {
	.minor = MISC_DYNAMIC_MINOR,
	.name = "lmj_textlcd",
	.fops = &lmj_textlcd_fops,
};

static int lmj_textlcd_init(void){
	printk("lmj_textlcd_init, \n");
	
	return misc_register(&lmj_textlcd_driver);
}

static void lmj_textlcd_exit(void){
	printk("lmj_textlcd_exit, \n");

	misc_deregister(&lmj_textlcd_driver);
	
}

module_init(lmj_textlcd_init);
module_exit(lmj_textlcd_exit);

MODULE_AUTHOR("Myeongji");
MODULE_DESCRIPTION("lmj_textlcd driver test");
